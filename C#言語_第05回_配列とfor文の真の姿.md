...
* for文には「真の姿」があり、次のように書く<br>
  <code><span class="hljs-keyword">for</span>(添字変数の宣言; 繰り返し条件; 添字変数の更新)<br>
  &emsp;{<br>
  &emsp;&emsp;繰り返したいプログラム<br>
  &emsp;}</code>
...
このように、同じ目的(例えば「敵の座標」とか)のデータが複数ある場合、配列を使うほうがずっと短く宣言できます。また、`{}`内にデータを追加するだけで、簡単にデータ数を増やせます。
...
コピペで済むぶん、for文を使わないほうが簡単に書けてよい気もします。しかし実は、for文には「真の姿」があるのです。真の姿を使えば、プログラムはもっと短く、読みやすくなります。

for文を「真の姿」にするには、`(; ; )`の部分に「繰り返しを制御するプログラム」を書きます。例えば、2.1節のプログラムは以下の形式になっていました。
...
for文の真の姿を使えば、このプログラムは次のように書き直せます。
...
サボテンを表示するプログラムを使って、for文の真の姿を練習しましょう。for文を真の姿に変えるには、以下の3段階の手順を踏みます。
...
<div align="center">
<strong>図解for文の真の姿</strong><br>
<img src="images/cpp2025_for_loop.png" width="50%" />
</div>
