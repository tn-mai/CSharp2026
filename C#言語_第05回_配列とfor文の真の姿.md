...
配列の要素を読み書きするには、`配列名[要素の番号]`のように書きます。番号は、先頭の要素が`0`で、次が`1`、その次が`2`、のようになります。この番号のことを「添字(そえじ)」といいます。例えば、以下のプログラムは「配列`enemyX`の`1`番目の要素」を意味します。
...
添字には「整数の変数」や「式」も指定できます。次の例では、「変数`b`から`1`を引いた数値」が添字になります。
...
配列を使うと、複数の変数をひとつにまとめられます。しかし、配列を使うプログラムはまとめられないので、添字ごとに普通の変数と同じプログラムを書くことになります。
...
この`for(;;)`は無限ループなので、変数`a`の値は無限に増え続けます。ところが、`sabotenX`の要素は`3`個しかありません。つまり、添字は`2`までしか増やせないのです。
...
このように、添字を変数にしてfor文と組み合わせると、何度も`Console.SetCursorPosition`や`Console.WriteLine`を書かなくて済みます。

### 2.2 for文の真の能力

添字の変数とfor文を使うと、同じプログラムを何度も書かなくても済みます。ですが、プログラムの行数は増えているし、for文とif文の両方が必要で、あまり読みやすくもありません。

コピペで済むぶん、for文を使わないほうが簡単に書けてよい気もします。しかし実は、for文には「真の能力」が隠されています。真の能力を使えば、プログラムはもっと短く、読みやすくなります。

for文を「真の能力」を解放するには、`(; ; )`の部分に「繰り返しを制御するプログラム」を書きます。例えば、2.1節のプログラムは以下の形式になっていました。
...
サボテンを表示するプログラムを使って、for文の真の能力を練習しましょう。真の能力を使うには、以下の3段階の手順を踏無必要があります。
...
`else`句と`break`文が無くなっている点に注目してください。これは、「真の能力」によって、条件が`false`になると、for文が自動的に`break`してくれるためです。
...
>「プロパティ」は「性質」や「資産」という意味です。C#のプロパティは、必要に応じて読み取りまたは代入を禁止できる「ちょっと特殊な変数」です。例えば、`Length`は「読み取り専用」で、代入はできません。`a.Length = 1;`のように書くとエラーになります。「複雑な型」は固有のプロパティを持つことが多いです。
...
for文の「真の能力」の練習として、ジャンプゲームのプログラムを「真の能力」を使うように書き換えましょう。`JumpGame`プロジェクトを開き、サボテンを表示するプログラムを次のように変更してください。
...
`sabotenX`配列を使っている、他のプログラムも「真の能力」に書き換えましょう。サボテンを動かすプログラムを、次のように変更してください。
...
for文の「真の能力」は、元のプログラムが長いほど、プログラムを短くする効果が高くなります。

続いて、衝突判定プログラムを「真の能力」で書き直しましょう。衝突判定プログラムを、次のように変更してください。
...
ですが、ここではfor文の「真の能力」を使えません。なぜなら、代入する数値が違うからです。「真の能力」は、「添字以外はすべて同じプログラム」に対してしか使えないのです。

とりあえず、2.4までのプログラムが書けたら、`>JumpGame`をクリックしてアプリを実行してください。for文を真の能力に書き換える前と全く同じ動作をしていたら成功です。

### 2.6 真の能力を使えるようにする

for文の真の能力は、「添字以外はすべて同じプログラム」でなくては使えません。裏を返せば、「添字以外はすべて同じプログラム」に変えられるなら、for文の真の能力を解放できるわけです。
...
あとは、for文の真の能力で書き換えるだけです。サボテンを右端に移動するプログラムを、次のように変更してください。
