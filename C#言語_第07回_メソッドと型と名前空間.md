...
これまで「命令」と呼んでいたものは、実はほとんどが「メソッド」です。今後は「命令」ではなく「メソッド」と呼ぶことにします。
...
先頭の`static`(スタティック)は、「属性キーワード」というものです。今は「メソッドの前にあったりなかったりする」くらいの認識でいてください。詳しい説明は別の機会にします。

`static`から後ろが「メソッドの定義」です。メソッドは`戻り型 メソッド名(パラメータ)`の形式で始まり、直後にメソッド・ブロックが続きます。

```c#
属性キーワード 戻り型 メソッド名(パラメータ)
{
  メソッド・ブロック
}
```

「戻り型(もどりがた)」は、 **メソッドが最終的に作り出すデータの種類** です。例えば整数の計算を行うメソッドでは、戻り型は`int`になるでしょう。詳しいことは別の機会に説明します。

`void`(ボイド)は「型がない」ことを意味する「特別な型」です。メソッドが終わるとき、何のデータも作り出さない場合に指定します。

パラメータは必要な場合だけ書きます。`Main`メソッドの場合、パラメータは不要ということです。

>`static void Main()`メソッドはC#プログラムの必須要素(ひっすようそ)です。コンソールアプリの場合、ビルドするときに、コンピューターが自動的に`static void Main()`を追加する仕組みになっています。

#### 型(かた)

メインメソッドを囲む`internal static class Program`(インターナル・スタティック・クラス・プログラム、「内部の静的な`Program`という名前の型」という意味)の部分は、「型(かた)」を作るブロックです。

型は`属性キーワード class 型名`の形式で始まり、型ブロックが続きます。`internal static`の部分が「属性キーワード」です。この部分は当面は無視してください。
...
`class`は「型」という意味で、ここからが「型の定義」になります。このプログラムの場合、型名(かためい)は`Program`(プログラム)です。この`Program`型は「アプリに必要なプログラムを入れる枠組み」となります。
