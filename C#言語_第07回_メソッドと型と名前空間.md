...
実際の描画(びょうが)は`DrawImage`(ドロー・イメージ、「画像を描く」という意味)メソッドで行います。`DrawImage`メソッドには、次のように4つのパラメータを指定します。

&emsp;**void DrawImage(画像変数, X座標, Y座標, 幅, 高さ);**
...
<code>OnPaint</code>メソッドの恐竜を描くプログラムの下に、<code>bmpSaboten</code>を描くプログラムを追加しなさい。
なお、サボテンの画像の大きさは「幅=64ドット、高さ=64ドット」です。
</pre>
...
+      g.DrawImage(bmpDino, dinoX, dinoY, 64, 64);
       g.DrawImage(bmpSaboten, 0, 0, 64, 64);
     }
```
