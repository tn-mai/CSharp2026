...
この、 **プログラムを「メソッドというかたまり」として覚えられるようにする** というのは、メソッドを作る大きな理由です。「ある処理を行う100行のプログラム」の内容を覚えておくのは大変ですが、「ある処理を行うメソッド」は、名前だけ覚えればいいのでずっと簡単になります。
...
「恐竜以外のプログラム」はちょっと混ぜすぎに感じるので、あとでちゃんと分けましょう。とりあえず、この3つの部分に分けてメソッド化してみましょう。
...
>**【インデントを直すには】**<br>
>プログラムを移動させると、インデント(各行の先頭にある空白のこと)がずれてしまうことがあります。数行なら手作業で直せばいいです。ですが、5行以上なら、Visual Studioの機能でぱぱっと直すのがよいでしょう。<br>
...
まずは、サボテンのプログラムから移動しましょう。メソッド名は`UpdateSaboten`(アップデート・サボテン、「サボテンを更新する」という意味)とします。
...
次に、作成した`UpdateSaboten`メソッドを実行します。プレイ中状態のプログラムに、`UpdateSaboten`メソッドを実行するプログラムを追加してください。
...
サボテンの移動を制御するプログラムを選択して、`Ctrl+X`で切り取ってください。
...
同様に、翼竜を動かすプログラムを移動させます。`UpdateSaboten`メソッドの定義の下に、`UpdatePtera`メソッドの定義を作成してください。
...
課題01と02が終わったら、`>DinoRun`ボタンをクリックしてアプリを実行してください。翼竜が上下に揺れながら移動していたら成功です。
...
背景を動かすプログラムをメソッド化したら、`>DinoRun`ボタンをクリックしてアプリを実行してください。背景が、プログラムの移動前と同じように動いていたら、恐竜以外のメソッド化は成功です。
...
それと、タイトル画面にはタイトルロゴを表示するものです。大きめのロゴ用フォントも追加しましょう。フォント変数を宣言するプログラムに、タイトルロゴ用の変数宣言を追加してください。変数名は`fontTitle`(フォント・タイトル)とします。
...
そして、フォントを読み込みます。`Main`メソッドのフォントを作成するプログラムに、タイトル用フォントを作成するプログラムを追加してください。
...
`fontTitle`を作るとき、第３のパラメータとして`FontStyle.Italic`(フォントスタイル・イタリック)を追加しています。フォントスタイルはフォントの見た目を少しだけ変える機能です。`Italic`は文字を斜めに傾けます。
...
メソッド名は`DrawText`(ドロー・テキスト)でいいでしょう。`TextRenderer`がなくなるので、短く書けるようになります。

さて、今回作るメソッドの目的は「`TextRenderer.DrawText`を短く書けるようにする」ことです。そのため、`TextRenderer.DrawText`に指定しているのと同じパラメータが必要です。

ひとつずつ説明するのは大変なので、とりあえず書いてみましょう。`UpdateTitle`メソッドブロックの下に、次のプログラムを追加してください。
...
パラメータの型は、`TextRenderer.DrawText`メソッドにカーソルを合わせて調べました。パラメータ名は分かりやすければ何でも構いません。

この`DrawText`メソッドのプログラムは、指定されたパラメータをそのまま`TextRenderer.DrawText`メソッドに再指定するだけのものです。

また、`new Point`をメソッド内で行うようにしたので、`DrawText`メソッドを使う側は座標を直接指定できるのも改善点です。

それでは、この`DrawText`メソッドを使ってみましょう。`OnPaint`メソッドにある、タイトル状態の表示プログラムを、次のように変更してください。
...
プログラムを変更したら、`>DinoRun`ボタンをクリックしてアプリを実行してください。変更前と同じようにタイトル画面が表示されたら成功です。
