...
* 「型」や「クラス」はデータの種類のことで、次のような種類がある
   ...
   <tr><td><font size=3>実数</font></td><td><font size=3>float</font></td><td><font size=3>フロート</td></tr>
...
* 変数を使うには「宣言(せんげん)」して「初期化」する必要がある<br>
  ...
  ```c#
  int number = 0;
  float real = 3.14f;    // float 型に実数を代入するには、末尾に f を付ける
  string text = "hello";
  ```
...
例えば、普段、掛け算や割り算を書くときは、`✕`や`÷`といった記号を使います。しかし、これらの記号はキーボードにありません。<br>
そこで、代わりの記号として、掛け算は`*`(アスタリスク)、割り算は`/`(スラッシュ)を使って書きます。

| 計算 | 数学 | プログラム |
|:----:|:----:|:----------:|
| 足し算 | <font size=3>＋</font>  | <font size=3>＋</font> |
| 引き算 | <font size=3>ー</font>  | <font size=3>ー</font> |
| 掛け算 | <font size=3>✕</font> | <font size=3>＊</font> |
| 割り算 | <font size=3>÷</font> | <font size=3>/</font> |
...
計算結果を画面に出力するには`Console.WriteLine`(コンソール・ライトライン)命令を使います。
...
なお、足し算と引き算、または掛け算と割り算のように、優先順位が同じ場合は、式の左側にある部分が優先されます。<br>
これは、次に説明する「整数と実数」が混ざった式で重要になります。
...
>**その他の演算子の優先順位**<br>
>C#言語ではさまざまな記号が使われますが、その全てに優先順位が定められています。<br>
>詳しくは https://learn.microsoft.com/ja-jp/dotnet/csharp/language-reference/operators などを参照。

### 1.3 整数と実数

C#言語では「整数」と「実数」を区別します。例えば、`3`や`4`のように「小数部を持たない数値」は「整数」になります。<br>
それに対して、`13.0`のように「小数部を持つ数字」は「実数」になります。
...
整数と実数では「実数が優先」されます。式に実数があらわれると、それ以降の計算は実数で行われます。<br>
上記のプログラムの`13.0 / (13 / 3)`という式の場合、`(13 / 3)`の部分は「整数同士の割り算」なので、答えは整数の`4`になります。<br>
その後`13.0 / 4`が実行されますが、これは「実数と整数の割り算」なので、答えは実数の`3.25`となります。

### 1.4 剰余(じょうよ)

整数の割り算では「あまり」が切り捨てられます。では「あまり」が知りたい場合はどうしたらよいでしょう。答えは「`%`(パーセント)記号を使う」です。
...
<code class="hljs-addition">+int playerHP = rand.Next(5) + 18;</code>

 string enemyName = "殺人ウサギ";
<code class="hljs-deletion">-int enemyHP = 16;</code>
<code class="hljs-addition">+int enemyHP = rand.Next(7) + 13;</code>
...
コンピューターの中には「メモリ」という「電子的なメモ帳」があります。<br>
このメモ帳のページに名前を付けると、プログラムからはそのページが「変数」として使えるようになります。<br>
つまり、変数は「名前の付いたメモ」といえます。

以下の画像は、適当なページに`number`という名前を付けて、`10`という数字を書き込んだ状態をあらわしています。

<img alt="変数をメモる" src="images/memo_int_number_10.png" width=200 />

プログラムからは、`number`という名前を使って、このページに書き込んだり、ページに書かれた内容を読み出すことができます。
...
変数の「型(かた)」は、「データの種類」のことです。例えば`int`(イント)型は「整数」、`string`(ストリング)型は文章が扱えます。<br>
そして、「変数の名前」のことを「変数名(へんすうめい)」と言います。

次のプログラムでは、「`int`型」で「名前が`number`」の変数を宣言し、「`57`という整数」を割り当てています。

```c#
int number = 57;
```

<img alt="変数宣言" src="images/memo_int_number_57.png" width=200 />
...
<img src="images/memo_int_number_assignment_57_to_10.png" width=420 /><br>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
<img src="images/memo_int_number_10.png" width=200 />

これで`number`という名前の変数に`10`が書き込まれます。このような`=`による割り当てを「代入(だいにゅう)」といいます。
また、この`10`のように、変数に書き込む数値や文章のことを、「データ」または「値（あたい）」といいます。

>**【C#の=記号は「代入」だけ】**<br>
> 算数や数学では`=`記号を「代入」と「等しい」の２つの意味で使います。しかし、C#の`=`記号の意味は「代入」だけです。<br>
> 「等しい」という意味を表すには`==`を使う必要があります。`==`記号については別のテキストで説明します。
...
`b = a`というのは「変数 a の内容を、変数 b に上書きする」という動作だと考えてください。

<img alt="aをbに代入" src="images/memo_int_assignment_a_to_b_1.png" width=420 /><br>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
<img alt="bがaと等しくなる" src="images/memo_int_assignment_a_to_b_2.png" width=420 /><br>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
<img alt="aに5を代入" src="images/memo_int_assignment_a_to_b_3.png" width=640 /><br>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
<img alt="bは10のまま" src="images/memo_int_assignment_a_to_b_4.png" width=420 /><br>

>より正確に言うなら、「変数`a`に割り当てたデータが読み出され、読み出されたデータが変数`b`に代入される」という流れになります。
...
「ある変数に何かを足したり引いたりしたい」ということはよくあります。<br>
これは次のように書くことができます。
...
これは「複合代入演算子(ふくごう・だいにゅう・えんざんし)」といって、四則演算の記号に続けて等号記号`=`を書きます。<br>
「演算記号と`=`記号のあいだに空白を入れてはいけない」点に注意してください。
...
名前が全く思いつかない場合は、とりあえず`a`とか`b`などと名付けても構いません。また、AIに名前を提案してもらうのも良い方法です。
...
型(かた)は「データの種類」をあらわすプログラミング用語です。型ごとにできること(機能)が決まっています。<br>
例えば`int`型は「整数の計算」ができます。`float`型は「実数の計算」ができます。
...
`string`型は、`"abc"`や`"Hello, World"`のように、単語や文章を`"`(ダブルクォーテーション)記号で囲ったデータを代入できます。<br>
`"`記号で囲ったデータのことを「文字列」といいます(これまでは「文章」と呼んでいました)。

`char`型は、`'a'`や`'-'`のように、1つの文字を`'`(シングルクォーテーション)記号で囲ったデータを代入できます。<br>
...
`float`型に実数を代入するときは、数値の後ろに文字`f`(エフ)を付けなくてはなりません。この`f`は`float`の頭文字です。

```c#
float a = 3.14f;
float a = 3.14F; // 大文字の F でも OK
```
...
表にあるように`double`型なら何も付ける必要はありません。ですが、ゲームでは、速度にすぐれる`float`型が多用されます。本テキストでも基本的には`float`型を使うので、「最後の`f`」を忘れないようにしてください。
...
数値を扱う型であれば、型が違っても混ぜて式を作れます。計算結果の型は「より大きな数値を表現できる側の型」になります。<br>
例えば、`int`型と`float`型の計算結果は`float`型になります。この機能を「型変換(かたへんかん)」といいます。

`string`型は少し特殊で、「加算」だけが可能です。減算や乗算、除算はできません。<br>
`string`型同士を加算すると、2つの文字列をつなげた新しい文字列が作られます。`string`以外の型を加算すると、その型を文章に変換して
...
また、文字列と`string`型は加算しかできません。引き算や掛け算、割り算をしようとするとエラーになります。
...
変数の型と、代入するデータの種類が違っていると、エラーになる場合があります。
...
このように、`(型の名前)データ`と書くと、データを`()`内に書いた「型」に変換できます。この変換を「キャスト」といいます。

変換先の型で表現できない部分は切り捨てられます。例えば、`int`型は小数点以下を表現できません。そのため、`(int)1.5f`と書いた場合、変換後のデータは、小数点以下が切り捨てられて`1`になります。

キャストできる型は、`int`や`float`のような数値型に限られます。例えば、`string`型は`int`型にキャストできません。
...
例えば、`Console.ReadLine`命令の結果は文字列です。計算に使いたい場合は`int.Parse`命令で`int`型に変換する必要があります。
...
なお、`+`記号を使って、文字列と数値を組み合わせて表示する場合、数値は自動的に文字列に変換されます。
