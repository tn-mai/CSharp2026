...
* 使用できるフォント名は、Windowsのスタートメニューの「設定→個人用設定→フォント」から調べられる
...
サボテンを配列にして数を増やすのは、コンソールアプリと同じ手順で実現できます。<br>
...
>「`}`が多くて分かりにくいな」と感じたら、その部分の一番下の`}`にコメントを書きます。「これなら、分かるかな」となったら、それ以上は書きません。「まだ分かりにくいな」という場合は、その上の`}`にもコメントを書きます。これを、分かるようになるまで繰り返します。
...
>ここでif文の条件式を4行に分けたのは、2行で書くには紙面の横幅が足りないからです。<br>
>書き写すときは、行を分ける必要はありません。

### 1.3 ゲームの展開に変化をつける

サボテンの数を増やしたとはいえ、まだゲーム内容はかなり単調です。<br>
これは、サボテンの出現する間隔が決まっているためです。そこで、出現間隔をランダムにしましょう。<br>
仮想キー番号を宣言するプログラムの下に、乱数の変数を宣言するプログラムを追加してください。
...
サボテンの出現位置を完全なランダムにしてしまうと、出現位置が重なってしまう場合があります。<br>
ランダムかつ重ならないように、次の方法を使うことにします。
...
まず、すべてのサボテンが画面外に出るのを待ちましょう。<br>
これは、「画面外に出たサボテンの数」を調べることで実現できます。<br>
...
PCにインストールされているフォントは、スタートメニューから
  設定→個人用設定→フォント
と選択することで調べられます。
</pre>
...
数字を覚えておくのは大変ですから、コンピューターにやらせましょう。<br>
...
恐竜の範囲は`(18,12)`～`(46,52)`です。これらの数値から、「翼竜の範囲」に対する「恐竜の範囲」の判定は以下のようになります。
...
それでは、恐竜とサボテンの衝突判定プログラムの下に、恐竜と翼竜の衝突判定プログラムを追加してください。
...
>`Sin`で構成されています。`MathF`は、`float`型
用の数学関数」がまとめられた名前空間です。

<pre class="tnmai_assignment">
<strong>【課題06 上下の揺れ幅を増やす】</strong>
翼竜の上下の揺れ幅がまだ少ないようです。
掛ける数値を40以上の適当な大きさに変更して、揺れ幅を大きくしなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題07 】</strong>
現在、翼竜の基本Y座標は400に固定されています。
基本Y座標をあらわす変数<code>pteraBaseY</code>を追加して、
右端に戻るたびに、ランダムに出現する高さが変わるようにしなさい。
</pre>
