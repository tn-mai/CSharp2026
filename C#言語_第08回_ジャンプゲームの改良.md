...
* メソッドを`.`記号でつないで描くことを「メソッドチェーン」という
* 衝突判定は、見た目より少し小さくする
* `Bitmap`などの複雑な型を配列にするには、個々の要素を`new`で作成する
* 2Dアニメーションは、一定時間ごとに画像を切り替えることで実現できる
* 利用可能なフォント名は、Windowsのスタートメニューにある「設定→個人用設定→フォント」から調べられる
...
そこで今回は、ジャンプゲームにさまざまな **要素の追加** や **手触りの改善** を行い、「ゲームとして面白い」という段階にすることを目指します。追加・改良点のリストは次のとおりです。
...
>ただし、すべてのブロックの終わりにコメントを入れるのは **やりすぎ** です。「カッコが多くて分かりにくいな」と感じたら、一番外側の閉じカッコにコメントを書きます。「まあ、分かるかな」となったら、そこで止めます。まだ分かりにくいようなら、その内側の閉じカッコにもコメントを書き、分かるようになるまで繰り返します。
...
まず乱数用の変数を宣言します。仮想キー番号を宣言するプログラムの下に、乱数の変数を宣言するプログラムを追加してください。
...
ですが、サボテンの出現位置を完全なランダムにしてしまうと、サボテンが重なってしまう場合があります。ランダムかつ重ならないように、次の方法を使うことにします。

1. すべてのサボテンが左の画面外に出てしまうまで待つ
2. サボテンの出現位置を40ドット単位で32箇所に分け、0～31の番号を振る
3. 番号をランダムに入れ替える(シャッフルする)
4. シャッフルした番号の先頭から3個を、新しいサボテンの出現位置とする

まず、すべてのサボテンが画面外に出るのを待ちましょう。これは、「画面外に出たサボテンの数」を調べることで実現できます。サボテンを右端に戻すプログラムを、次のように変更してください。
...
このfor文が終了したとき、`passedSabotenCount`(パスド・サボテン・カウント)が「サボテンの個数」以上なら、すべてのサボテンが画面外に出ています。

すべて画面外に出ていたら、新しい出現位置を決めます。`0`～`31`の番号列(数列)を作るには`Enumerable`(イニューメラブル、「数えられるもの」という意味)型を使います。

今回は、`Enumerable`型の以下のメソッドを使います。

* `Range`(レンジ、「範囲」という意味)メソッド: 連続した数値を作る
* `ToArray`(トゥ・アレイ、「配列にする」という意味)メソッド: 数列を配列に変換する

それから、配列をシャッフルするには`Random`型の`Shuffle`(シャッフル)メソッドを使います。
...
`Range`メソッドのパラメータは「最初の数値」と「作成する個数」です。今回は`0`から`31`まで32個の数値を作りたいので、`(0, 32)`としています。

`.`(ドット)記号を使ってメソッドをつなぐと、前のメソッドの戻り値に対して、次のメソッドを呼び出すことができます。この書きかたを「メソッド・チェーン」といいます。
...
>「32」という数値は、1画面の横幅1280を、40ドットで割ったものです。範囲が狭すぎるとパターンが少なくなって飽きやすいです。逆に広すぎるとサボテンがなかなか出現しないので退屈してしまいます。
...
Windowsのディスプレイ設定には「拡大/縮小」という項目があります。これは、画面のドットが細かすぎて文字が読みづらい場合に、表示を調整するための機能です。

`Font`クラスを作成するときは、この「拡大/縮小」を考慮した大きさを指定する必要があります。そうしないと、文字の大きさが「拡大/縮小」の設定によって変わってしまいます。

拡大縮小の設定は、フォームの`DeviceDpi`(デバイス・ディーピーアイ)プロパティで調べられます。つまり、「フォーム作成後」でないと拡大縮小の設定を調べられない、ということです。

>`DPI`は「ドット・パー・インチ」の略語で、「長さ1インチに含まれるドット数」という意味です。なお、1インチは2.54cmです。

しかし現在は、フォントはフォームの作成より前に読み込んでいます。そこで、順番を「フォームを作成してからフォントを読み込む」ように変更します。
...
`null`(ヌル)は「データが存在しない」ことをあらわすキーワードです。`null`の後ろに`!`(びっくりマーク)記号を入れることに注意してください。この`!`記号は、

&emsp;**本当はデータが必要だけど、今だけ無くてもいいことにしたい**

という場合に、`null`の直後に付けます。

基本的に、C#の変数は初期値を設定して使うものです。そのため、「何も無い」状態を作ろうとすると「それ、危ないですよ」と **警告** されます(警告されるだけで、実行はできる)。`!`記号を付けると、「私(プログラマ)は危険を理解して使っています」と表明したことになり、警告されなくなります。

>実は`null`と`!`のあいだに、空白は不要です(`null!`でOK)。上のプログラムで空白を入れているのは、見間違いを防ぐためです。

次に、フォームを作成して表示するプログラムの下に、フォントを読み込むプログラムを追加してください。
...
`DeviceDpi`プロパティの値は、拡大/縮小の設定が100%の場合は`96`、125%の場合は`120`(=96x1.25)、150%の場合は`144`(=96*1.5)になります。

変数`dpiScale`(ディーピーアイ・スケール)は、「拡大/縮小を打ち消すための拡大率」です。Windows標準、つまり100%のDPIは`96`なので、「`96`を現在のDPIで割った値」となります。
...
それから、`fontScore`(フォント・スコア)変数にフォントを読み込みます。`Main`メソッドにあるフォントを作成するプログラムに、次のプログラムを追加してください。
...
得点などの普段から表示される文字の場合、あまり主張しないデザインのほうが読みやすいです。そこで、手書き風の`Segoe Print`(シーゴ・プリント)フォントを選んでみました。

>**【フォントの種類を変えるには】**<br>
>Windows OSに標準でインストールされているフォントは、スタートメニューから「設定→個人用設定→フォント」と選択することで調べられます。気になったフォント名をクリックすると、同じタイプのフォント一覧が表示されます。そこに書いてある名前が、`Font`型に渡すフォント名になります。
...
```diff
     ...
     private static float[] sabotenX = { 1280.0f, 1780.0f, 1830.0f }; // サボテンのX座標
     private static float sabotenY = 580.0f;  // サボテンのY座標
+    private static int sabotenSpeed = 6.0f; // サボテンの速度
...
<strong>【課題01 サボテンの速度制限】</strong>
サボテンの速度は無限に速くなります。ですが、あまり速すぎるとゲームになりません。
サボテンの移動速度を更新したあと、最高速度が「1/60秒ごとに20ドット」になるように制限しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題02 サボテンの速度を戻す】</strong>
...
画像をアニメーションさせると、キャラクターが「生きている」という感覚が生まれて、没入感を高められます。恐竜をアニメーションさせて、没入感を高めましょう。

2Dゲームの基本的なアニメーションは、複数の画像を定期的に切り替えることで実現します。そのためには、複数の画像を読み込まねばなりません。これは配列を使うのが簡単です。
...
2枚目の画像のX座標を、画像サイズぴったりの1280ドットではなく、わずかに小さい1279.5ドットにしている点に注意してください。これは、ぴったりにすると、拡大/縮小が100%でない場合に、画像のあいだに隙間ができることがあるからです。0.5ドットだけ重ねることで、隙間をなくせます。

プログラムを追加したら、`>DinoRun`ボタンをクリックしてアプリを実行してください。空白が無くなって、背景がずっと流れていくようになっていたら成功です。

<pre class="tnmai_assignment">
<strong>【課題03 雲を表示する】</strong>
画像ファイル<code>assets/images/cloud.png</code>を読み込んで、空に表示するプログラムを追加しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題04 雲を動かす】</strong>
課題03で表示した雲を、ゆっくりと左へ動かし、完全に画面外に消えたら右端に移動させるプログラムを追加しなさい。
</pre>
...
翼竜の再出現の間隔は、サボテンの2倍にしてみました。翼竜が再出現するまでの時間を長くすることで、翼竜がサボテンと一緒に出たり、出なかったりします。これには、ゲームの単調さを減らす狙いがあります。
...
これらの数値から、例えば衝突範囲の左側の条件式は`pteraX + 14.0f > dinoX - 18.0f`となります。それでは、恐竜とサボテンの衝突判定プログラムの下に、恐竜と翼竜の衝突判定プログラムを追加してください。
...
`MathF.Sin`メソッドの結果は`-1`～`1`の範囲です。揺れ幅がたったの2ドットでは動いているように見えないので、20倍して`-20`～`20`の範囲の動きにしています。

プログラムを追加したら、`>DinoRun`ボタンをクリックしてアプリを実行してください。翼竜が上下に動きながら移動していたら成功です。

>`MathF.Sin`という名前は、数学を意味する`Math`(マス)、`float`型の`F`(エフ)、サイン関数の`Sin`で構成されています。`MathF`は、`float`型
用の数学関数」がまとめられた「名前空間」です。

<pre class="tnmai_assignment">
<strong>【課題05 上下の揺れ幅を増やす】</strong>
翼竜の上下の揺れ幅がまだ少ないようです。40以上の適当な大きさを設定して、揺れ幅を大きくしなさい。
</pre>
