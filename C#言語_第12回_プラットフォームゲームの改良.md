...
* 新しいクラスを作らなくても、既存のクラスを流用できる場合がある
* クラスの使いかたが変わったら、クラスの名前を変えることを検討する
* 何度も使いそうなプログラムは、インスタンスメソッドにする
* 「ある機能に必要なデータ」が場面によって変わる場合、クラスにまとめる
...
トゲブロックだけでは、プレイヤーに選択を迫る状況のバリエーションを増やしにくいです。<br>
「左右に移動する敵」を追加して、バリエーションを増やしやすくしましょう。

<div align="center"><img alt="敵" src="images/12_hedgehog_image.png" /></div>

というわけで、「移動する敵」のクラスを作ります。このクラスに必要なフィールドを考えてみましょう。

「移動する敵」は、当然ながら移動します。移動するためには「座標」フィールドが必要です。<br>
そして、移動先にブロックがあったら向きを変えたいです。となると、「向き」フィールドも必要です。<br>
また、ブロックの端から落ちたりするかもしれません。落ちるには、「ジャンプ状態」フィールドが必要です。<br>
アニメーションさせたいなら、「アニメ制御タイマー」フィールドも必要です。

こうして見ると、「移動する敵」クラスのフィールドは、プレイヤークラスとほとんど同じになりそうです。<br>
...
<div align="center"><img alt="名前変更メニュー" width="50%" src="images/12_rename_menu.png" /></div>

名前の入力欄が表示されるので、`Character`と入力してEnterキーを押してください。

<div align="center"><img alt="名前変更ダイアログ" width="50%" src="images/12_rename_dialog.png" /></div>
...
さて、クラス名は変わりましたが、クラスのコメントは「プレイヤークラス」とか「Playerクラス」のままです。<br>
プログラムとコメントが一致しないと、あとで、敵がバグるような変更をしてしまうかもしれません。<br>
そこで、コメントを修正します。`Character`クラスの先頭のコメントを、次のように書き直してください。
...
次に、`Character`クラスブロックの終わりにあるコメントを、次のように書き直してください。
...
これで、クラスの機能とコメントが一致するようになりました。

### 1.2 敵を表示する

プレイヤーは1体だけですが、敵は複数同時に出せたほうがいいです。<br>
...
<div align="center"><img alt="敵を描く" width="50%" src="images/12_draw_hedgehog.png" /></div>

### 1.3 移動する敵を移動させる

さて、「移動する敵」は移動するべきです。そこで、敵を現在向いている方向に移動させましょう。<br>
敵のプログラムがプレイヤーのプログラムと混ざらないようにしたいです。<br>
そこで、敵の移動プログラムは、プレイヤーの処理が全て終わった後、プレイヤーのゴール判定の後に書きます。
...
ただし、今の`UpdateAnimeTimer`メソッドはプレイヤー用に書かれていて、敵には使えません。

そこで、メソッドで使うデータを **クラスのメンバーフィールドにする** ことにします。名前は以下のようにします。
...
次に、追加したフィールドを使うように、`UpdateAnimeTimer`メソッドを変更してください。
...
追加した`AnimeTimestep`と`AnimeLenght`は、プレイヤーと敵で異なります。<br>
そこで、この2つのフィールドに値を代入するメソッドを作ります。<br>
...
次に、すぐ下にある「ジャンプ中ならジャンプ状態を更新する」プログラムを範囲選択して、<br>
`Ctrl+X`で切り取ってください。
...
敵を落下させるプログラムの下に、敵とブロックの衝突を判定するfor文を追加してください。
...
+      {
+        Block block = blockList[b]; // 判定するブロック
+
+        // 上昇中でなければ着地を判定する
...
次に、足元の座標とブロックの座標を計算しましょう。<br>
敵の足の座標は、とりあえずプレイヤーと同じ`12.0f`～`52.0f`の範囲とします。<br>
敵とブロックの衝突を判定するforブロックの中に、次のプログラムを追加してください。
...
+          // 敵の足元の座標を計算
+          Box boxFoot = new(
+            enemy.X + 12.0f, enemy.X + 52.0f,
+            enemy.Y + 64.0f, enemy.Y + 64.0f);
+
+          // ブロックの着地可能範囲の座標を計算
+          Box boxGround = new(
+            block.X * 64.0f, block.X * 64.0f + block.Size * 64.0f,
...
続いて、足とブロックの衝突を判定します。<br>
ブロックの座標を計算するプログラムの下に、衝突を判定するプログラムを追加してください。
...
<div align="center"><img alt="敵が落ちて着地" width="50%" src="images/12_hedgehog_falls_and_stands_on.png" /></div>
...
+        Box boxBlock = new(
+          block.X * 64.0f, block.X * 64.0f + block.Size * 64.0f,
+          block.Y * 64.0f, block.Y * 64.0f + 64.0f);
+
+        // 敵の左側の座標を計算
+        Box boxLeft = new(
...
敵の左の座標は、ブロックに密着する直前に反転するように、画像の端から4ドット内側にしてみました。<br>
`Direction`フィールドは右向きが`1.0f`、左向きが`-1.0f`なので、符号を変えれば向きも変わります。

<pre class="tnmai_assignment">
<strong>【課題01 右側の衝突判定を追加する】</strong>
左側の衝突判定プログラムの下に、右側の衝突判定プログラムを追加しなさい。
右側の座標は<code>60.0f</code>とすること。
</pre>

課題ができたら`>PenguinsJump`ボタンをクリックしてアプリを実行してください。<br>
...
-          enemy.X + 12.0f, enemy.X + 52.0f,
-          enemy.Y + 64.0f, enemy.Y + 64.0f);
+        Box boxFoot = Box.MakeFootBox(enemy, 12.0f, 52.0f, 64.0f);
...
-          enemy.Y + 12.0f, enemy.Y + 52.0f);
+        Box boxLeft = Box.MakeSideBox(enemy, 4.0f, 12.0f, 52.0f);
+        Box boxRight = Box.MakeSideBox(enemy, 60.0f, 12.0f, 52.0f);
...
ですが、「キャラクターの衝突判定用の長方形を作る」メソッドはまだ作っていません。<br>
なければ作りましょう。
...
終わるのが早すぎるので、ゴールするたびに新しい画面に切り替わるようにしましょう。

「画面」という単語は意味が複数あって使いにくいです。<br>
そこで、今後は「切り替わる各画面」のことを **レベル** と呼ぶことにします。

レベルごとにブロックや敵の配置を変えたいので、「レベルに必要なデータをまとめたクラス」を作ります。<br>
名前は`LevelData`(レベル・データ)とします。`Box`クラスの定義の下に、次のプログラムを追加してください。
...
レベルデータにおける敵やプレイヤーのデータは、出現する座標さえ分かれば十分です。<br>
ジャンプ状態やアニメーションは動作中にのみ必要なフィールドで、レベルデータには不要です。<br>
そこで、座標をあらわす`Point`(ポイント)型を使うことにしました。

また、これまで、敵、プレイヤー、ゴールの座標は「ドット単位」で指定していました。<br>
そのため、「(5,3)のブロックの上に敵を配置したい」場合、`(320, 192)`を指定する必要がありました。

ですが、レベルデータでは全ての座標を「ブロック単位」で扱うことにします。<br>
つまり、画面を20x11のマスに区切り、マス目の位置で座標を指定します。<br>
「(5,3)のブロックの上に敵を配置したい」場合、`(5,2)`と書けるので、分かりやすくなります。
...
続いて、レベルデータの配列を作ります。配列名は`levelDataList`(レベルデータ・リスト)とします。<br>
...
単位がそろっているほうが、他のブロックとの位置関係が分かりやすいからです。

最初のレベルデータには、以前に作ったブロックやトゲブロックの配列の内容を使います。<br>
...
同じ手順で、トゲブロックの中身も移動します。<br>
トゲブロックの配列の内容を選択して、`Ctrl+X`で切り取ってください。
...
<pre class="tnmai_assignment">
<strong>【課題02 下向きトゲブロック配列の内容をレベルデータに移動する】</strong>
<code>downSpikeList</code>配列の内容を切り取って、
レベルデータ配列の「下向きトゲブロック配列」の部分に貼り付けなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題03 敵の配列の内容をレベルデータに移動する】</strong>
<code>enemyList</code>配列の内容を切り取って、
レベルデータ配列の「敵の配列」の部分に貼り付けなさい。
レベルデータは「64x64を単位とする座標系」に揃えているので、数値を64で割ること。
</pre>
...
実は、**newに続けてクラス名を書くほうが、本来の書きかた** です。<br>
ただし、以下の場合は`new`とクラス名を省略できます。
...
+
+      // 敵の座標から敵キャラクターを作成
+      enemyList = new Character[data.EnemyList.Length]; // 新しい配列を作る
+      for (int a = 0; a < enemyList.Length; a += 1)
+      {
...
<div align="center"><img alt="レベル2に切り替わる" width="50%" src="images/12_level_2.png" /></div>

**課題提出について**<br>
以下の課題を可能な限り完了させたら、次の手順でファイルを提出してください。<br>
課題評価は、未提出なら0点、ファイル提出で50点、課題07まで完了していれば100点、です。<br>
**提出期限は９月９日（水）** までとします。<br>
&emsp;1. 以下のフォルダに「自分のフルネーム」の新しいフォルダを作成<br>
&emsp;&emsp;`Share2/※講師フォルダ※/田中真依講師/2026年度前期_C#言語_課題提出先`<br>
&emsp;2. PenguinsJumpゲームのプロジェクトフォルダにある`Program.cs`ファイルを、<br>
&emsp;&emsp;「自分のフルネーム」フォルダにコピー

<pre class="tnmai_assignment">
<strong>【課題04 死亡状態でもApplyLevelDataを実行する】</strong>
...
<pre class="tnmai_assignment">
<strong>【課題06 レベル３のデータを作る】</strong>
レベルデータ配列にレベル３を追加し、レベル１や２とは別のブロック配置データを作りなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題07 効果音、BGMを再生する】</strong>
NuGetを使って<code>SFML.Audio</code>パッケージを追加し、効果音やBGMを再生しなさい。
  - 効果音: ジャンプしたとき、着地したとき、やられたとき
  - BGM: プレイ中、クリア、死亡
</pre>

<pre class="tnmai_assignment">
<strong>【課題08 タイトル画面を作る】</strong>
タイトル画面を追加しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題09 さらに要素を追加する】</strong>
以下の要素を、できる限り多くゲームに追加しなさい。
  - レベル数を４つ以上に増やす
  - 操作説明画面を作る
  - 横向きのトゲブロック
  - 最終レベルクリア後にゲームクリア画面を表示する
  - 新しい敵その１(足元に床がない場合に向きを変える)
  - 新しい敵その２(空中を左右、または上下に移動する)
  - プレイヤーが下を通過すると落下する岩・鍾乳石
  - プレイヤーの攻撃(敵を倒したり、ブロックを壊したりできる)
  - 一定時間で消え、また一定時間で現れることを繰り返す床ブロック
  - 一定時間で床ブロックから出たり戻ったりするトゲブロック
</pre>
...
頭で想像したマップをプログラムのデータにするのは難しいです。<br>
実際に図を書いて、試行錯誤するほうが効率的です。<br>
以下のマス目にブロックやゴール、トゲを書き込んで、マップデザインに役立ててください。
