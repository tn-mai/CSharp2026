...
&emsp;前々回: 地形の表示, プレイヤーとゴールの表示, プレイヤーの移動とジャンプ, プレイヤーの着地<br>
&emsp;前回: プレイヤーの向き, プレイヤーの上側の判定, プレイヤーの側面の判定, ゴール判定, クリア状態,<br>
&emsp;&emsp;&emsp;&ensp;トゲブロック, ゲームオーバー状態<br>
&emsp;**今回: 敵の表示, 敵の衝突判定, アニメーション, タイトル状態, ステージ数を増やす**
...
> **【インスタンス】**<br>
> `new`で作る変数のことを「インスタンス、「実例」「具体例」という意味)といいます。<br>
> インスタンス・メソッドは、インスタンスがないと実行できません。スタティック・メソッドはインスタンスがなくても実行できます。
...
メソッド名は`MakeGroundBox`(メイク・グラウンド・ボックス、「地表の衝突判定ボックスを作る」という意味)とします。`MakeBlockBox`メソッドの定義の下に、次のプログラムを追加してください。
...
メソッド名は`MakeFootBox`(メイク・フット・ボックス)と`MakeSideBox`(メイク・サイド・ボックス)とします。
...
これで、プレイヤーの座標計算は書き直しできました。続いて、敵の座標計算を書き直します。<br>
...
続いて、敵の左側と右側の座標計算を、`MakeSideBox`メソッドを使うように書き直してください。
...
プログラムが書けたら`>PenguinsJump`ボタンをクリックしてアプリを実行してください。<br>
計算式を`MakeFootBox`と`MakeSideBox`で書き直す前と同じように、ブロックにぶつかったり乗り降りできたら成功です。
...
レベルデータの配列を作ります。配列名は`levelDataList`(レベルデータ・リスト)とします。<br>
...
パラメータに配列を指定する場合は、以下の形式で書きます。

&emsp;`new クラス名[]{ 要素1, 要素2, 要素3, ... }`

基本はこの書きかたですが、配列変数の宣言では、特別に`new クラス名[]`を省略できるようになっています。<br>
以下の2つの書きかたは、どちらも同じ配列を作成します。<br>
&emsp;`Block[] blockListA = { new(1, 1, 1), n32(2, 2, 2) };`<br>
&emsp;`Block[] blockListB = new Block[]{ new(1, 1, 1), n32(2, 2, 2) };`
...
ゲームで使う変数は`Program`クラスのメンバーなので、`ApplyLevelData`も`Program`クラスのメンバーにします。<br>
`Program.cs`の一番下にある`DrawText`メソッドの定義の下に、次のプログラムを追加してください。

```diff
     // 画面に文字列を描く
     private static void DrawText(Graphics g, string t, Font f, int x, int y, Color c)
     {
       TextRenderer.DrawText(g, t, f, new Point(x, y), c);
     }
+
...
現在、最初のブロックや敵の配置は、以下の2つの場所に書かれています。

1. `Program`クラスの`blockList`配列や`spikeList`配列
2. `levelDataList`配列

同じデータが2ヶ所にあると、データを書き換えたくなったときに面倒です。<br>
そこで、配置データとして`levelDataList`だけを使うようにします。

まずブロック配列とトゲブロック配列の宣言を、次のように変更してください。
...
-      new(19.0f, 9.0f, 1)
-    };
+    private static Block[] blockList = null !;

     // トゲブロックの配列
-    private static Block[] spikeList = {
-      new(16.0f, 9.0f, 2), new(5.0f, 5.0f, 1), new(14.0f, 2.0f, 1),
-    };
+    private static Block[] spikeList = null !;

     // ゴールクラス
     public class Goal
```

次に、敵の配列を次のように変更してください。
...
-      new(384.0f, 64.0f),
-    };
+    private static Character[] enemyList = null !;

     // 長方形クラス
     public class Box
```

これで、アプリの起動直後はレベルデータが未設定の状態になりました。
...
敵のアニメ情報の反映は`ApplyLevelData`メソッドで実行しているので削除しています。
