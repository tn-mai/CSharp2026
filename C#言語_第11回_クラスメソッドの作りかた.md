...
// Console.WriteLine(Test.Number); // NG. Number はクラスの共有メンバーではない
```
...
`static`を付けるのは、Visual Studioが自動的に作成する`Program`クラスのように、 **アプリ全体でひとつだけ必要なクラス** だけです。
...
ドアの上部が円形なので、右端の図のように、ぎりぎり触れていないのにゴールになる状況がありえます。ですが、意図して狙わない限りこの状況にはなりにくいです。それに、大抵のプレイヤーは、得をする条件に文句を言いません。ゴール範囲などはちょっと大きいくらいがいいのです。
...
せっかくゴールしたのに、文字が表示されるだけ、というのも味気ないです。<br>
そこで、ゴールの文字が上から降ってくる演出を追加しましょう。

位置を変化させるには変数が必要です。<br>
「ゴールの文字のY座標」なので、名前は`goalLogoY`(ゴール・ロゴ・ワイ)とします。<br>
...
つまり、Y座標を`0`にすると、画面の一番上に表示されます。<br>
画面に映っていない状態から降りてきてほしいので、画面外の座標を代入しています。
...
このような、「行動に危険がなさすぎて緊張感を持てないゲーム」はあまり面白くありません。<br>
...
続いて、アニメーション用のフィールドを更新するメソッドを定義します。
...
メソッドブロックでは、フィールド名を使って操作するフィールドを指定します。
