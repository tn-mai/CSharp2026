...
最後に、`OnPaint`メソッドの中で`PaintGoal`メソッドを呼び出します。<br>
...
-      DrawText(g, "G O A L", fontHeading, 400 + 8, 250 + 8, Color.DarkRed);
-      DrawText(g, "G O A L", fontHeading, 400, 250, Color.Red);
+      DrawText(g, "G O A L", fontHeading, 400 + 8, (int)goalLogoY + 8, Color.DarkRed);
+      DrawText(g, "G O A L", fontHeading, 400, (int)goalLogoY, Color.Red);
     } // PaintGoalメソッドの終わり
...
現在、ゴール状態と死亡状態の両方に、プレイヤーを初期状態に戻すプログラムが存在します。<br>
