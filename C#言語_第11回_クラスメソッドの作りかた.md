...
&emsp;前回: 地形の表示, プレイヤーとゴールの表示, プレイヤーの移動とジャンプ, プレイヤーの着地

&emsp;今回: プレイヤーの向き, プレイヤーの上側の判定, プレイヤーの側面の判定, ゴール判定, クリア状態, トゲブロック, ゲームオーバー状態

&emsp;次回: プレイヤーのアニメーション, 敵の表示, 敵の衝突判定, タイトル状態, 横スクロール, ステージ数を増やす
...
画像を描く`DrawImage`メソッドには、追加パラメータとして「表示するサイズ」を渡せます。

```c#
void DrawImage(画像変数, X座標, Y座標, Xサイズ, Yサイズ);
```

このとき、マイナスのサイズを指定すると画像が反転します。Xサイズがマイナスの場合は水平方向に反転し、Yサイズがマイナスの場合は垂直方向に反転します。
...
考えてみれば「上昇中に着地」なんてできないはずです。そこで、「下降中だけ着地を判定する」ことにします。
...
この`Box`クラスに、衝突判定を行うメソッドを追加します。「衝突する」は英語で`Collide`(コライド)というので、メソッド名は疑問形にして`IsCollide`(イズ・コライド)としましょう。
...
このif文をコピーして、`IsCollide`メソッドに貼り付けます。もちろんエラーが報告されますが、予想通りなので構いません。
...
頭の判定は1点だけなので、ボックス同士の衝突判定はやりすぎです。そこで、1点と衝突判定を行うメソッドも追加しましょう。機能は同じなので、メソッド名と戻り型は同じでいいでしょう。`IsCollide`メソッドの定義の下に、1点用の新しい`IsCollide`メソッドを追加してください。
...
+    // 戻り値: true  衝突している
+    //         false 衝突していない
+    public bool IsCollide(float x, float y)
+    {
+      if (x > Left && x < Right &&
+          y >= Top && y < Bottom)
+      {
...
それでは、`Box`クラスと`IsCollide`メソッドを使って、4つの衝突判定を書き換えましょう。まず、ブロックの上下左右の座標を計算するプログラムを、`Box`クラスで置き換えてください。
...
次に、頭とブロックの判定を`IsCollide`メソッドで置き換えてください。
...
続いて、足元とブロックの判定を`IsCollide`メソッドで書き換えてください。
...
-        float footY = player.Y + 65.0f;
+        Box foot = new(
+          player.X + 12.0f, player.X + 52.0f,
+          player.Y + 65.0f, player.Y + 65.0f);
+
+        // ブロックの上端だけのボックスを計算
+        Box ground = new(
+          block.Left, block.Right,
+          block.Top, block.Top + 20.0f);

         // 足が、ブロックの上側 20 ドットの範囲に入ったら、着地したことにする
-        if (footR > blockL && footL < blockR &&
-            footY > blockT && footY < blockT + 20.0f)
+        if (ground.IsCollide(foot))
         {
           player.Y = blockT - 64.0f; // プレイヤーをブロックの上に強制移動
           player.IsJumping = false;  // ジャンプしていない状態にする
```

足元の判定ではブロック全体ではなく、ブロックの上部20ドットだけが判定の範囲になります。そのため、`block`変数は使えません。そこで、新しく`ground`(グラウンド)変数を作っています。

最後に、左右の判定を`Iscollide`メソッドで置き換えます。
...
+    if (boxGoal.IsCollide(boxPlayer))
+    {
+      gameState = gsGoal;
+    }
...
文字を描くにはフォントが必要なので、フォントを読み込んでおきます。<br>
...
+
+    // フォント
+    private static Font font = new("Courier New", 64.0f, FontStyle.Bold);
...
+        {
+          g.DrawImage(bmpSpike, (spikeList[a].X + b) * 64.0f - px, spikeList[a].Y * 64.0f - py);
+        }
+      }
...
+      if (boxSpike.IsCollide(boxPlayer))
+      {
...
```diff
       PaintPlay(g);
       DrawText(g, "GOAL", font, 440 + 8, 300 + 8, Color.DarkRed);
       DrawText(g, "GOAL", font, 440, 300, Color.Red);
     } // PaintGoalメソッドブロックの終わり
+
+    // 死亡状態を更新する
...
+    {
+      PaintPlay(g);
+      DrawText(g, "YOU DIED", font, 360 + 8, 250 + 8, Color.DarkRed);
+      DrawText(g, "YOU DIED", font, 360, 250, Color.Red);
...
`Player`クラスの定義に、`Init`メソッドの定義を追加してください。
