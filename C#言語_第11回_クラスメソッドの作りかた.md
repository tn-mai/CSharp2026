...
ブロックの下からジャンプすると、ブロックをすり抜けてブロックの上に着地できてしまいます。<br>
そういうゲーム性もありえますが、今回の`PenguinsJump`アプリでは、ちゃんと頭をぶつけられるようにします。
...
頭が少しだけめりこむようにすると、「ぶつかった！」という感触が分かりやすくなります。

<div align="center"><img alt="" width="15%" src="images/10_player_hitbox_head.png" />[頭の判定]</div>
...
<div align="center"><img alt="" width="15%" src="images/10_player_hitbox_side.png" />[左側と右側の判定]　　　</div>

プログラムが書けたら`>PenguinsJump`ボタンをクリックしてアプリを実行してください。<br>
ブロックに横からぶつかってもすり抜けなくなっていたら成功です。

それから、「頭が上のブロックにぎりぎりぶつからないように」ジャンプしてみてください。すると、プレイヤーはブロックを避けるように、するりと横移動します。頭の判定を小さくしたのは、この不思議と納得感ある挙動にするためです。

さて、すべての判定を組み合わせると、衝突判定は次のようになります。

<div align="center"><img alt="" width="15%" src="images/10_player_hitbox_all.png" />[足元、頭、左右側の判定]</div>
...
+    {
+      // トゲブロックの上下左右の座標を計算
+      Block spike = spikeList[a];
+      Box boxSpike = new(
+        spike.X * 64.0f + 4.0f, (spike.X + spike.Size) * 64.0f - 8.0f,
+        spike.Y * 64.0f + 16.0f, spike.Y * 64.0f + 64.0f);
+
...
以下の3つのプログラムを追加して、画像を逆さまにしたトゲブロックを作成しなさい。
  ...
  1. <code>spikeList</code>の定義の下に、「下向きトゲブロックの配列」を追加
     名前は<code>downSpikeList</code>とすること
  2. トゲブロックを描くプログラムの下に、下向きトゲブロックを描くプログラムを追加
  3. トゲブロックとの衝突プログラムの下に、下向きトゲブロックとの衝突判定プログラムを追加
     下向きトゲブロックとプレイヤーが衝突したら、死亡状態にすること
</pre>
