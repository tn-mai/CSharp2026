...
クラス定義の`{`と`}`の間で宣言された変数のことを「メンバー・フィールド(member field)」、あるいは単に「メンバー」といいます。

>クラス定義には、変数だけでなく「メソッド」、「プロパティ」、「イベント」を含められます。これらについては別の機会に説明します。

#### アクセス修飾子

「アクセス修飾子」は **メンバーの利用権** を制御する機能で、基本的には以下のどちらかを指定します。
...
アクセス指定子は**省略可能**です。アクセス指定子を省略すると、自動的に`private`に設定されます。

>`private`を指定したメンバーは、「メンバー・メソッド」や「メンバー・プロパティ」でのみ利用できます。これらについては別の機会に説明します。
...
変数名は英語で書くのが基本ですが、英語だと意味が分かりにくい場合があります。こういう場合、「コメントで変数の意味を説明する」ことが多いです。<br>
コメントを書く位置に決まりはないのですが、次のように、変数名の後ろに書くことが多いです。
...
class Dog {
  ...
  public float Friendship = 0.7f; // なつき具合
}
```

### 1.3 クラス変数の宣言と初期化

クラスは「複雑な型」に分類されます。そのため、クラスの変数を宣言するには、`new`命令を使って次のように書きます。
...
if (myDog.Friendship >= 0.5f)
{
  Console.WriteLine(myDog.Name + "はまあまあなついています");
}
```

**実行結果**<br>
&emsp;きなこの3年後の年齢は6歳です<br>
&emsp;きなこはまあまあなついています
...
class Dog {
  ...
  // コンストラクタ(初期化専用のメソッド)
  public Dog(string name, string breed, int age, int gender, float friendship)
  {
    Name = name;
    Breed = breed;
    Age = age;
...
```c#
属性パラメータ クラス名(パラメータ・リスト)
{
  メンバーにパラメータを代入するプログラム
}
```

コンストラクタの名前は **クラス名と同じ** でなくてはなりません。また、コンストラクタの戻り値は常に「クラス自身」です。そのため、コンストラクタには「戻り型」を書きません。

コンストラクタの属性パラメータには`public`(パブリック)を指定します。`public`属性を指定すると「クラスブロックの外でも使える」ようになります。

このコンストラクタを使うと、`yourDog`変数は次のように初期化できます。

```c#
Dog yourDog = new("わたげ", "マルチーズ", 4, 1, 0.8f);
```

このように、クラスにコンストラクタを書いておけば、配列と似た書きかたでクラス変数を初期化できます。違いは、`new`命令が必要なこと、丸カッコを使うこと、の2点です。

### 1.6 クラスを使う場面

クラスは「種類の異なる複数のデータをまとめて扱う」ものです。実際に利用する場面では「あるものごとに関連するデータをまとめて扱う」ために使われます。

例えば、前の章で挙げた`Dog`クラスは「犬に関連するデータをまとめた型」です。ゲームでは、「物体の座標」や「アイテムデータ」、「敵データ」などをクラスにすることが多いです。

アイテムや敵ごとにデータが数個しかないゲームなら、クラスにまとめる意義は少ないです。例えば前回作ったジャンプゲームでは、敵となるサボテンはX座標だけで管理できました。

しかし、翼竜にはX座標とY座標、そしてアニメーションタイマーの3つのデータがありました。とはいえ、前回の翼竜は一匹しか表示しなかったので、クラスを使う意義は弱かったのです。もしも何匹も表示したい場合は、クラスを使うとプログラムが読みやすくなります。
...
翼竜の数を変えたくなったとき、基本型の配列では`pteraX`、`pteraY`、`pteraAnimeTimer`の3つを個別に変更する必要があります。<br>
しかし、クラスを使った例のほうは、`ptera`配列だけを変更すれば済みます。そのため、「配列のどれかを変更し忘れる」といったミスは起きようがありません。

`Ptera`クラスのコンストラクタが、アニメーションタイマーを設定していない点に注意してください。コンストラクタで代入しなかったメンバーには、メンバーの宣言で指定した初期値が代入されるからです。つまり、`AnimeTimer`メンバーには`0.0f`が代入されるわけです。

それから、「動きの違う翼竜を増やしたい」と思ったときも、次のように新しい配列をひとつ増やすだけです(もちろん、動きのプログラムは別に必要です)。
...
とはいえ、これくらいの例であれば、どちらの方法でも、動きの違う翼竜のプログラムを書くのは難しくないでしょう。無理にクラスを使わなくても作れるプログラムは、たくさんあります。ですが、上の`Ptera`型のように「関連する複数のデータがある」場合、クラスを使うほうが書きやすくなります。

<div style="page-break-after: always"></div>

## 2. ジャンプゲームを作る

今回から3回かけて、1画面版のスーパーマリオのようなゲームを作ります。<br>
プレイヤーは左右移動とジャンプを使って空中ブロックを飛び移り、ゴールのドアを目指します。<br>
各回では以下の内容を予定しています。

&emsp;今回: 地形の表示, プレイヤーとゴールの表示, プレイヤーの移動とジャンプ, プレイヤーの着地

&emsp;次回: プレイヤーの向き, プレイヤーの上側の判定, プレイヤーの側面の判定, ゴール判定, ゲームクリア状態

&emsp;次々回: トゲ地形の表示, トゲ地形の衝突判定, プレイヤーのやられ状態, ゲームの再開, タイトル状態
...
「プロジェクト名」は`JumpGame`(ジャンプ・ゲーム)としてください。「場所」はデスクトップのC#フォルダを選択してください。「ソリューションとプロジェクトを同じディレクトリに配置する」にチェックを入れて、「次へ」をクリックしてください。

「追加情報」は特に変更不要です。何も変えずに、右下の「作成」ボタンをクリックしてください。プロジェクト用のファイルが作成された後、プロジェクトが開きます。

Windowsフォームアプリの場合、最初に表示されるのは「フォーム」のデザイン画面です。「フォーム」は「外観、体型」といった意味で、Windows OSでは「さまざまな部品を配置できる親ウィンドウ」を指します。
...
設定を変更したら、Visual Sutidoの上部に表示される`>JumpGame`ボタンをクリックして、アプリを実行してください。何もないウィンドウが表示されたら成功です。`X`ボタンでウィンドウを閉じてください。
...
さて、前に作成した`DinoRun`アプリでは、画像の表示はぜんぶ`OnPaint`メソッドで行っていたため、少し分かりにくくなっていました。

そこで、今回作成する`JumpGame`では、絵を描くメソッドもゲーム状態ごとに分けます。これも、とりあえず「プレイ状態」のメソッドだけ定義します。`UpdateDead`メソッドの定義の下に、「プレイ状態の描画メソッド」を追加してください。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。ウィンドウが選んだ色で塗りつぶされていたら成功です。
...
1. Visual Studioで`JumpGame`プロジェクトを開く
2. ウィンドウ最上部にある「デバッグ」項目をクリックして開く
3. デバッグメニューの一番下にある「JumpGameのデバッグ プロパティ」をクリック
...
+      new(0.0f, 10.0f, 20), new(8.0f, 8.0f, 2), new(11.0f, 7.0f, 5),
+    };
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。画面にブロックが表示されていたら成功です。

<div align="center"><img alt="" width="50%" src="images/10_draw_blocks.png" /><br>[ブロックを表示]</div>

<pre class="tnmai_assignment">
<strong>【課題01 ブロックを増やす】</strong>
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。青いプレイヤー画像が表示されていたら成功です。

<pre class="tnmai_assignment">
<strong>【課題02 ゴールを表示する】</strong>
以下の手順を参考にゴールクラスを作成し、画面にゴール画像を表示しなさい。

  1. <code>blockList</code>の定義の下に、ゴールをあらわす<code>Goal</code>クラスを定義する。メンバーは<code>float</code>型の<code>X</code>座標と<code>Y</code>座標の2つ。
  2. <code>Goal</code>型の変数を宣言する。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。矢印キーの左右を押したとき、プレイヤー画像が左右に移動したら成功です。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。スペースキーを押したとき、プレイヤーがずっと上に移動続けたら成功です。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。スペースキーを押してジャンプしたとき、プレイヤーが途中から落下しはじめたら成功です。
...
+      float footY = player.Y + 65.0f; // 足のY座標(左右共通)
+
...
キャラクター画像の横幅は64ドットですが、両足は画像の端より内側にあります。そのため、端から12ドット内側を「足元の座標」としています。また、「足のすぐ下」を調べたいので、Y方向には64ではなく65を足しています。

それから、ブロック側の着地判定の範囲は「ブロックの上から20ドット」にしています。<br>
ブロックの横や下からぶつかっても「着地」と判定させないためです。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。スペースキーを押してジャンプし、ブロックに着地できたら成功です。

### 2.16 着地できないことがある

少し高いところからジャンプすると、ブロックに着地できないときがあります。これは落下速度が速すぎるためです。ゲームは1/60秒ごとに着地を判定するので、1/60秒ごとの落下量が20ドットを越えていると、着判定の20ドットをすり抜けてしまう場合があるのです。
...
+      if (player.JumpSpeed < 1200.0f)
+      {
+        player.JumpSpeed = 1200.0f;
+      }
     }
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。スペースキーを押してジャンプし、ブロックをすり抜けなくなっていたら成功です。

### 2.17 足元にブロックがない場合

現在のところ、空中にあるブロックの端を歩いて越えても落下しません。これは「足元にブロックがない」という状態を判定してないからです。

この判定のために、「なにかに着地したか？」をあらわす変数を追加します。変数名は`isGrounded`(イズ・グラウンデッド、「着地している？」という意味)とします。

プレイヤーとブロックの接触を判定するプログラムに、次のプログラムを追加してください。
...
```diff
     ...
     // プレイヤーとブロックとの接触
+    bool isGrounded = false; // なにかに着地したら true
     for (int a = 0; a < blockList.Length; a++)
     {
```

そして、ブロックに着地したら`isGrounded`変数を`true`にします。ブロックに着地するプログラムに、次のプログラムを追加してください。

```diff
         player.Y = blockT - 64.0f; // プレイヤーをブロックの上に強制移動
         player.IsJumping = false;  // ジャンプしていない状態にする
         player.JumpSpeed = 0.0f;   // ジャンプ速度を 0 にする
+        isGrounded = true;         // ブロックに着地した
       }
     }
   } // UpdatePlayの終わり
```

最後に、`isGrounded`変数を調べて「足元にブロックがない」ことを判定します。ブロックを調べるfor文が終わったとき、`isGrounded`が`false`のままなら、つまり，
「すべてのブロックを調べても着地が起きなかった」なら、「足元にブロックがない」と判定できます。

<div style="page-break-after: always"></div>

ブロックとの接触を調べるfor文の下に、次のプログラムを追加してください。
...
プログラムが書けたら`>JumpGame`ボタンをクリックしてアプリを実行してください。歩いて空中のブロックの端を越えたとき、プレイヤーが落下したら成功です。
