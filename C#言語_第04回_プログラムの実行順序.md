...
`&&`(アンド・アンド)演算子は「論理積(ろんりせき)演算子」と呼ばれ、２つの条件式が **どちらも真** のときだけ、式全体が真となります。

`||`(オア・オア)演算子は「論理和(ろんりわ)演算子」と呼ばれ、２つの条件式の **どちらか片方が真、または両方とも真** の場合に、式全体が真となります。

`!`(ノット、通称「びっくり」)演算子は「論理否定(ろんりひてい)演算子」と呼ばれ、条件式の意味を逆にします。例えば、`!(x == y)`は`x != y`と同じ意味になります。<br>
`bool`型の変数に対して、`!x`のように使われることが多いです(`!x`は`x == false`と同じ意味)。
...
>**【a == b == c ではなく a == b && b == c と書く】**<br>
>C#で「`a`と`b`と`c`は等しい」を表現したいとき、数学のように`a == b == c`とは書けません。<br>
>C#では、この式は、`(bool)(a == b) == c`と認識されます。<br>
>つまり、「`a`と`b`を比較して、その結果を`c`と比較する」という意味になってしまうのです。<br>
>C#で「`a`と`b`と`c`は等しい」を表現するには、`a == b && b == c`と書きます。
...
続いて、地面を表示します。地面は「レンガ」の絵文字を横に12個並べて表現することにします。<br>
...
地面を表示するプログラムの下に、次のプログラムを追加してください。

>`🌵`は「さぼてん」と入力して変換するか、漢字の`木`や、`△`記号などで代用してください。
...
+    Console.WriteLine("  "); // 絵文字の幅は半角空白2個
+
     sabotenX = 22.0f; // 右端に戻す
   }
 } // forの終わり
...
さて、プレイヤーがジャンプできる条件は、「ジャンプ中ではない」と「スペースキーが押されている」の2つです。<br>
このように複数の条件がある場合は、多重if文を使うか、`&&`演算子で条件を結合します。<br>
今回は練習を兼ねて`&&`を使います。スペースキーの状態を調べるプログラムを、次のように変更してください。
...
「繰り返し1回にかかる時間」はプログラムの長さによって変わりますが、とりあえず`0.001`秒としました。
